...
//    for (Int_t q=Ev1stSpill1st.Index;q<TOF_.triggeredTofhirEv.size(); q++) {
    for (Int_t q=100;q<101; q++) {
...
//
        ...
        matchEff=0;
        for(int k=0; k<trackerTree->GetEntries(); k++) {
            ...
            if (1) cout<<"k = "<<k<<"  time is= "<<TrackerBCOThisEvent *144e-9 * ClockScaleFactor<< " => dif= "<<trackerTime<<"\n";

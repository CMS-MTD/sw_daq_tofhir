...
#define SEARCHWINDOWSIZE 5000 //in ps units  (5000 was changed to 15000 after an optimization study)
...
int main(int argc, char* argv[]){
  ...
  anatree->SetBranchAddress("step1",&step1_);
  anatree->SetBranchAddress("step2",&step2_);
...
//  for (Int_t i=0;i<50000; i++) {
  ...
  for (Int_t q=index_firstEventInSpill;q<TOFHIR_TriggerIndices.size(); q++) {
   ...
   // cout <<channelID <<"  "<<triggerChannelID<<"\n";
	...
	if (DEBUG==0) cout << "MATCHED : " << event << " : " << TOFHIRTriggerTimestamp << "   | "<< trackerTime<<"\n";	
	NMatchedTracks++;

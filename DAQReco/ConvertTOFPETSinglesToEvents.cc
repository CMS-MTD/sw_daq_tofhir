...
int main(int argc, char* argv[]){
    ...
    //string singleFile= "/home/daq/2020_02_cmstiming_BTL/TOFHIR/RecoData/v1/RecoWithTracks/run"+runNumber+"_s.root";
    TFile *tofhirFile=new TFile(inputFileName,"read");
    TTree *tofhirTree = (TTree*)tofhirFile->Get("data");
    if( tofhirTree != NULL ) cout << "\n>>> got TOFHIR tree from file " << inputFileName << " with " << tofhirTree->GetEntries() << " entries" << endl;
    else exit(-1);
    
    TOFHIR TOF_(tofhirTree);
    
    //=====================Open fast Tracker input files=========================
    //string trackingFileFast = "/home/daq/2020_02_cmstiming_BTL/Tracks/Run"+runNumber+"_CMSTiming_FastTriggerStream_converted.root";
    TFile *trackerFileFast = new TFile(inputTrackerFileName,"READ");
    TTree *trackerTreeFast = (TTree*)trackerFileFast->Get("CMSTiming");
    if( trackerTreeFast != NULL ) cout << ">>> got track tree from file " << inputTrackerFileName << " with " << trackerTreeFast->GetEntries() << " entries" << endl;
    else exit(-1);
    
    TRACKER TRK_Fast(trackerTreeFast);
    
    //=====================Open Slow Tracker input files=========================
    string trackingFileSlow = "/home/daq/2020_02_cmstiming_BTL/Tracks/Run"+runNumber+"_CMSTiming_SlowTriggerStream_converted.root";
